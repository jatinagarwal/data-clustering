...
object RunLSA {
  def main(args: Array[String]) {
    ...
    /* 'numTerms' is size of feature vector i.e number of terms in a document
    ...
    /* 'sampleSize' implies sample size on which lsa algorithm should run */
    
    val conf = new SparkConf().setAppName("Wiki LSA")
    conf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    /* Configuration settings for spark is done here*/

    val sc = new SparkContext(conf)
    /* Decalaring the spark spark context object for performing transformations and actions on RDDs*/

    val (termDocMatrix, termIds, docIds, idfs) = preprocessing(sampleSize, numTerms, sc)
    /* In this step we call preprocessing method for generating term document matrix from raw text doucements */

    termDocMatrix.cache()
    /* term document matrix is cached */

    val mat = new RowMatrix(termDocMatrix)
    /* Transforming term documents matrix into row matrix for computing SVD */

    val svd = mat.computeSVD(k, computeU=true)
    /* Singular valuar ddecomposition is applied on the row matrix form of term-document matrix */

    println("Singular values: " + svd.s)
    /* Printing singular values on to the console */
  ...
  def preprocessing(sampleSize: Double, numTerms: Int, sc: SparkContext)
    ...
    // val pages = readFile("hdfs://localhost:54310/Wikipedia/", sc)
    //   .sample(false, sampleSize, 11L)

    // val plainText = pages.filter(_ != null).flatMap(wikiXmlToPlainText)    
   
    val dataFiles = "sampleJava" // Should be some directory on your system
   
    val codeData = sc.wholeTextFiles(dataFiles).cache()
    /* Reading and disriburitng files in the local directory into (documentName, documentContent) pairs */
 
    codeData.persist() // Prsisting data into memeory for processing

    val codeDataWithOutComments = codeData.mapValues{inp => 
      val regex = """(/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*+/)|(//.*)""".r //Regular expression to identify comments in a Java Code
      val specialChar = """[^a-zA-Z\n\s\#]""".r // Regular expression to remove speciall characters from any give text
      val commentsRemoved = regex.replaceAllIn(inp,"")
      specialChar.replaceAllIn(commentsRemoved, " ")
    } 

    val stopWords = sc.broadcast(loadStopWords("keywords.txt")).value

    val lemmatized = codeDataWithOutComments.mapPartitions(iter => {
